...
public class Utils : MonoBehaviour {
    ...
    static public ePosistion CalculatePosition(Vector2 me, Vector2 target)
    {
        ePosistion ePos = ePosistion.None;

        if(target.x<me.x)
        {
            ePos = ePosistion.Left;
        }
        else if(target.x > me.x)
        {
            ePos = ePosistion.Right;
        }
